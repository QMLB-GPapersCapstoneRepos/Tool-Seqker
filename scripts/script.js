...
function validateForm() {
   var valid=isNotEmpty("name", "Please enter your name!")
      ...
      && isChecked("genome", "Please check a genome!")
      //&& isChecked("task", "Please check a task!")
      && isLengthMinMax("sequence", "Please enter a valid 23 base pairs long sequences!", 23, 23)
      && isDNAsequence("sequence", "Please enter a valid 23 base pairs long sequences!")
      && isChecked("pam", "Please check a PAM type!")
      && isChecked("output1", "Please check an Output preference!")
      && isChecked("output2", "Please check an Output preference!");
   return (valid);
}
...
function isDNAsequence(inputId, errorMsg) {
   var inputElement = document.getElementById(inputId);
   var errorElement = document.getElementById(inputId + "Error");
   var inputValue = inputElement.value.trim();
   var isValid = (inputValue.search(/^[AGCTagct]+$/)!== -1);
   showMessage(isValid, inputElement, errorMsg, errorElement);
   return isValid;
}
