<!doctype html>
<html>
<head>
   <meta charset="utf-8">
   <meta name="viewport" content="width=device-width" />
   <link rel="stylesheet" type="text/css" href="stylesheets/styles.css">
   <link rel="shortcut icon" href="images/favicon.gif">
   <title>References</title>
   <script type="text/javascript" src="scripts/script.js"></script>
</head>

<body>
   <header>
      <i><h1>SeqKer</h1></i>
      <i><h2>Kernel based tool for TF binding prediction</h2></i>
   </header>

   <div class="page width">
      <section id="menu">
      <ul>
         <li id="menu-home"><a href="index.html"><span>Home</span></a></li>
         <li id="menu-about"><a href="about.html"><span>About</span></a></li>
         <li id="menu-userguide"><a href="userguide.html"><span>User Guide</span></a></li>
         <li id="menu-references" style="background: #89A34E;border-top-left-radius:20px;border-top-right-radius: 20px;">
            <a href="references.html" style="color: #4A5928;text-shadow: 0 -2px 2px #bdcd9c;"><span>References</span></a>
         </li>
         <li id="menu-contact"><a href="contact.html"><span>Contact</span></a></li>
      </ul>
      </section>
      <article align="center">
         <h2>To be updated</h2>
      </article>
   </div>

   <center><h3>Developed in <a href="http://www.cs.virginia.edu/yanjun/index.htm">CS Dept @ UVa</a></h3></center>

</body>
<html>
