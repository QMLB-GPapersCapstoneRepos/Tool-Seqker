...
/*   Date: 16.12.97                                                     */
/*                                                                      */
/*   Revised by Yanjun Qi for read self-created kernel                   */
/*   Date: 21.03.2014                                                   */
/*                                                                      */
/************************************************************************/
     ...
     /* plug in you favorite kernel */                          
{
  ...
  //printf("In custom_kernel \n") ;    

  /* each line that is formatted as:  
     -1 1:30037 2:906 3:528 4:354 #1  
     label index:value index:value ... #indexOfInstance
  */

  //printf("ao:%s\n", a->userdefined) ;    
  index_a = atoi(a->userdefined); 
  //printf("a:%d\n", index_a) ;    

  //printf("bo:%s\n", b->userdefined) ;    
  index_b = atoi(b->userdefined); 
  //printf("a:%d\n", index_b) ;    
  ...
  if (( index_a != 0 ) && ( index_b !=0 )) 
      ...
      while (bj->wnum) 
	{
      ...
      if ( abs(k_a - k_b) > 0.01 )
	{
	  //printf("not symmetry kernal value: %f %f", k_a, k_b) ;    
	  return(0); 
	}  
      else 
	{  
	  //printf(" symmetry kernal value: %f %f", k_a, k_b) ;    
	  return((double)k_a);
	} 
    }
  else 
